...
test.group('Users | destroy', (group) => {
  ...
  test('ADMIN should deactivate any user', async ({ client }) => {
  ...
  test('USER should deactivate himself', async ({ client }) => {
  ...
  test('USER should not deactivate another user', async ({ client }) => {
    const user = await User.create({
      fullName: 'User 1',
